let currentEditingPlaylist="",tempPlaylistSongs=[];function loadPlaylist(e){if("default"===e){var t=[`${URL}OP1 - Redo.mp4`,`${URL}ED1 - STYX HELIX.mp4`,`${URL}OP2 - Paradisus - Paradoxum.mp4`,`${URL}ED2 - Stay Alive.webm`,`${URL}OP3 - Realize.mp4`,`${URL}ED3 - Memento.mp4`,`${URL}OP4 - Long shot.mp4`,`${URL}ED4 - Believe in you.webm`,`${URL}OP5 - Reweave.mp4`,`${URL}ED5 - NOX LUX.mp4`,],l=[`${URL}STRAIGHT BET.mp4`,`${URL}Bouya no Yume yo.mp4`,`${URL}Memories.mp4`,`${URL}White White Snow.mp4`,`${URL}Requiem of Silence.mp4`,`${URL}Wishing.mp4`,`${URL}Yuki no hate ni Kimi no na wo.mp4`,`${URL}Door.mp4`,`${URL}What you don't know.mp4`,`${URL}I Trust You.mp4`,];videoUrls=t,newvideoUrls=l,playVideo(videoUrls[0]),showToast(`Đ\xe3 kh\xf4i phục danh s\xe1ch ph\xe1t mặc định.`);return}let a=(JSON.parse(localStorage.getItem("playlists"))||{})[e];a&&a.songs&&(playVideo((videoUrls=a.songs.map(e=>convertToUrl(e)))[0]),showToast(`Đ\xe3 tải danh s\xe1ch ph\xe1t "${e}" với ${a.songs.length} b\xe0i h\xe1t.`))}function convertToUrl(e){let t=MASTER_SONGS.find(t=>t.title===e);return t?`${URL}${t.filename}`:"ED2 - Stay Alive"===e||"ED4 - Believe in you"===e||"S1 Ending"===e?`${URL}${e}.webm`:`${URL}${e}.mp4`}function populateSidebarPlaylists(){let e=JSON.parse(localStorage.getItem("playlists"))||{},t=document.getElementById("dynamic-playlists");t.innerHTML="",Object.keys(e).forEach(l=>{let a=e[l];if(!a.songs||0===a.songs.length)return;let n=document.createElement("li");n.innerHTML=`
    ...
    `,l.addEventListener("click",e=>{e.target===l&&closePlaylistEditor()}),l}function clearPlaylist(e){confirm(`X\xf3a tất cả b\xe0i h\xe1t khỏi danh s\xe1ch "${e}"? Lưu \xfd: Danh s\xe1ch trống kh\xf4ng thể được lưu.`)&&(tempPlaylistSongs=[],refreshModalContent())}function saveNameEdit(){let e=document.getElementById("playlist-name-input"),t=e.value.trim();if(!t){showToast("T\xean danh s\xe1ch ph\xe1t kh\xf4ng được để trống!","error");return}let l=document.querySelector("#playlist-name-display span");l.textContent=t,l.style.color="#10b981",setTimeout(()=>{l.style.color=""},1e3),cancelNameEdit(),showToast('T\xean đ\xe3 được thay đổi. Nhớ nhấn "Lưu thay đổi" để ho\xe0n tất!',"info")}function getCurrentPlaylistName(){let e=document.getElementById("playlist-name-edit");if(e&&"none"!==e.style.display){let t=document.getElementById("playlist-name-input");return t?t.value.trim():currentEditingPlaylist}let l=document.querySelector("#playlist-name-display span");return l?l.textContent.trim():currentEditingPlaylist}function startEditingName(){let e=document.getElementById("playlist-name-display"),t=document.getElementById("playlist-name-edit"),l=document.getElementById("playlist-name-input");e.style.display="none",t.style.display="flex",setTimeout(()=>{l.focus(),l.select(),l.addEventListener("keydown",handleNameEditKeydown)},100)}function handleNameEditKeydown(e){"Enter"===e.key?(e.preventDefault(),saveNameEdit()):"Escape"===e.key&&(e.preventDefault(),cancelNameEdit())}function cancelNameEdit(){let e=document.getElementById("playlist-name-display"),t=document.getElementById("playlist-name-edit"),l=document.getElementById("playlist-name-input");l.removeEventListener("keydown",handleNameEditKeydown),l.value=currentEditingPlaylist,t.style.display="none",e.style.display="flex"}document.addEventListener("DOMContentLoaded",async()=>{await loadMasterSongs(),populateSidebarPlaylists()});let draggedIndex=null;function dragStart(e,t){draggedIndex=t,e.dataTransfer.effectAllowed="move",e.target.style.opacity="0.5"}function dragOver(e){e.preventDefault(),e.dataTransfer.dropEffect="move"}function dragDrop(e,t){if(e.preventDefault(),null!==draggedIndex&&draggedIndex!==t){let l=tempPlaylistSongs[draggedIndex];tempPlaylistSongs.splice(draggedIndex,1),tempPlaylistSongs.splice(t,0,l),refreshModalContent()}draggedIndex=null,e.target.style.opacity="1"}function dragEnd(e){e.target.style.opacity="1",draggedIndex=null}function filterAvailableSongs(){let e=document.getElementById("song-search").value.toLowerCase(),t=document.getElementById("available-songs"),l=t.querySelectorAll("[data-song-title]");l.forEach(t=>{let l=t.getAttribute("data-song-title").toLowerCase();l.includes(e)?t.style.display="flex":t.style.display="none"})}function generateAvailableSongsList(e){let t=MASTER_SONGS.filter(t=>!e.includes(t.title));return 0===t.length?`
    ...
    `).join("")}let MASTER_SONGS=[];async function loadMasterSongs(){try{let e=await fetch("Assets/data/songsMetadata.json"),t=await e.json();MASTER_SONGS=[...t.openings,...t.endings,...t.inserts,...t.specials].sort((e,t)=>{let l={opening:1,ending:2,insert:3,special:4};return l[e.type]!==l[t.type]?l[e.type]-l[t.type]:e.order-t.order})}catch(l){return showToast("Gặp lỗi khi lấy th\xf4ng tin video:","error",l),getAllAvailableSongsLegacy()}}function getAllAvailableSongs(){return MASTER_SONGS.map(e=>e.title)}function getAllAvailableSongsLegacy(){let e=[],t=[...videoUrls,...newvideoUrls];return t.forEach(t=>{let l=cleanVideoSrcName(t);e.includes(l)||e.push(l)}),e.sort()}function addSongToCurrentPlaylist(e){tempPlaylistSongs.includes(e)||(tempPlaylistSongs.push(e),refreshModalContent())}function removeSongFromCurrentPlaylist(e){tempPlaylistSongs=tempPlaylistSongs.filter(t=>t!==e),refreshModalContent()}function refreshModalContent(){let e=document.getElementById("available-songs"),t=document.getElementById("current-playlist");if(e&&t){e.innerHTML=generateAvailableSongsList(tempPlaylistSongs),t.innerHTML=generateCurrentPlaylistList(tempPlaylistSongs);let l=document.querySelector('button[onclick*="savePlaylist"]');l&&(0===tempPlaylistSongs.length?(l.disabled=!0,l.className=l.className.replace("bg-blue-500","bg-gray-400"),l.className=l.className.replace("hover:bg-blue-600","cursor-not-allowed")):(l.disabled=!1,l.className=l.className.replace("bg-gray-400","bg-blue-500"),l.className=l.className.replace("cursor-not-allowed","hover:bg-blue-600")))}}function closePlaylistEditor(){let e=document.getElementById("playlist-editor-modal");e&&(e.style.opacity="0",setTimeout(()=>{e.remove(),window.isEditingPlaylist=!1},300))}function savePlaylist(){let e=getCurrentPlaylistName(),t=currentEditingPlaylist;if(!e){showToast("T\xean danh s\xe1ch ph\xe1t kh\xf4ng được để trống!","error");return}if(0===tempPlaylistSongs.length){let l=JSON.parse(localStorage.getItem("playlists"))||{};delete l[t],localStorage.setItem("playlists",JSON.stringify(l)),closePlaylistEditor(),populateSidebarPlaylists(),showToast(`Đ\xe3 x\xf3a danh s\xe1ch trống "${t}"`,"warning");return}let a=JSON.parse(localStorage.getItem("playlists"))||{};if(e!==t){if(a[e]){showToast(`Danh s\xe1ch "${e}" đ\xe3 tồn tại!`,"error");return}a[e]={songs:[...tempPlaylistSongs],created:a[t].created,description:a[t].description||"",modified:new Date().toISOString()},delete a[t],showToast(`Đ\xe3 đổi t\xean từ "${t}" th\xe0nh "${e}" v\xe0 lưu th\xe0nh c\xf4ng!`)}else a[e].songs=[...tempPlaylistSongs],a[e].modified=new Date().toISOString(),showToast(`Đ\xe3 lưu th\xe0nh c\xf4ng danh s\xe1ch ph\xe1t ${e}`);localStorage.setItem("playlists",JSON.stringify(a)),populateSidebarPlaylists(),closePlaylistEditor()}