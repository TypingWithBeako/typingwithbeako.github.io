let currentEditingPlaylist="",tempPlaylistSongs=[];function loadPlaylist(e){if("default"===e){var t=[`${URL}OP1 - Redo.mp4`,`${URL}ED1 - STYX HELIX.mp4`,`${URL}OP2 - Paradisus - Paradoxum.mp4`,`${URL}ED2 - Stay Alive.webm`,`${URL}OP3 - Realize.mp4`,`${URL}ED3 - Memento.mp4`,`${URL}OP4 - Long shot.mp4`,`${URL}ED4 - Believe in you.webm`,`${URL}OP5 - Reweave.mp4`,`${URL}ED5 - NOX LUX.mp4`,],l=[`${URL}STRAIGHT BET.mp4`,`${URL}Bouya no Yume yo.mp4`,`${URL}Memories.mp4`,`${URL}White White Snow.mp4`,`${URL}Requiem of Silence.mp4`,`${URL}Wishing.mp4`,`${URL}Yuki no hate ni Kimi no na wo.mp4`,`${URL}Door.mp4`,`${URL}What you don't know.mp4`,`${URL}I Trust You.mp4`,];videoUrls=t,newvideoUrls=l,playVideo(videoUrls[0]);return}let a=(JSON.parse(localStorage.getItem("playlists"))||{})[e];a&&a.songs&&(videoUrls=a.songs.map(e=>convertToUrl(e)),currentIndex=0,playVideo(videoUrls[0]))}function convertToUrl(e){return"Stay Alive"===e||"Believe in you"===e||"S1 Ending"===e?`${URL}${e}.webm`:`${URL}${e}.mp4`}function populateSidebarPlaylists(){let e=JSON.parse(localStorage.getItem("playlists"))||{},t=document.getElementById("dynamic-playlists");t.innerHTML="",Object.keys(e).forEach(l=>{let a=document.createElement("li");a.innerHTML=`
            <div class="flex items-center p-2 pl-11 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-blue-100">
                <button onclick="loadPlaylist('${l}')" class="flex-1 text-left">
                    ${l} (${e[l].songs.length})
                </button>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 flex-1 min-h-0">
                ...
                <div class="flex flex-col min-h-0">
                    <div class="flex items-center justify-between mb-4 flex-shrink-0">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">B\xe0i h\xe1t c\xf3 sẵn</h3>
                ...
                <div class="flex flex-col min-h-0">
                    <div class="flex items-center justify-between mb-4 flex-shrink-0">
                        <h3 class="text-lg font-semibold text-gray-900 dark:text-white">Danh s\xe1ch hiện tại</h3>
    ...
    `,l.addEventListener("click",e=>{e.target===l&&closePlaylistEditor()}),l}function clearPlaylist(e){confirm(`X\xf3a tất cả b\xe0i h\xe1t khỏi danh s\xe1ch "${e}"?`)&&(tempPlaylistSongs=[],refreshModalContent())}document.addEventListener("DOMContentLoaded",populateSidebarPlaylists);let draggedIndex=null;function dragStart(e,t){draggedIndex=t,e.dataTransfer.effectAllowed="move",e.target.style.opacity="0.5"}function dragOver(e){e.preventDefault(),e.dataTransfer.dropEffect="move"}function dragDrop(e,t){if(e.preventDefault(),null!==draggedIndex&&draggedIndex!==t){let l=tempPlaylistSongs[draggedIndex];tempPlaylistSongs.splice(draggedIndex,1),tempPlaylistSongs.splice(t,0,l),refreshModalContent()}draggedIndex=null,e.target.style.opacity="1"}function dragEnd(e){e.target.style.opacity="1",draggedIndex=null}function filterAvailableSongs(){let e=document.getElementById("song-search").value.toLowerCase(),t=document.getElementById("available-songs"),l=t.querySelectorAll("[data-song]");l.forEach(t=>{let l=t.getAttribute("data-song").toLowerCase();l.includes(e)?t.style.display="flex":t.style.display="none"})}function generateAvailableSongsList(e){let t=getAllAvailableSongs();return t.map(t=>{let l=e.includes(t);return`
