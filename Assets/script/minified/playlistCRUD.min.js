let currentEditingPlaylist="",tempPlaylistSongs=[],PLAYLIST_VERSION="v2";function loadPlaylist(e){if("default"===e){var t=[`${URL}OP1 - Redo.mp4`,`${URL}ED1 - STYX HELIX.mp4`,`${URL}OP2 - Paradisus - Paradoxum.mp4`,`${URL}ED2 - Stay Alive.webm`,`${URL}OP3 - Realize.mp4`,`${URL}ED3 - Memento.mp4`,`${URL}OP4 - Long shot.mp4`,`${URL}ED4 - Believe in you.webm`,`${URL}OP5 - Reweave.mp4`,`${URL}ED5 - NOX LUX.mp4`,],l=[`${URL}STRAIGHT BET.mp4`,`${URL}Bouya no Yume yo.mp4`,`${URL}Memories.mp4`,`${URL}White White Snow.mp4`,`${URL}Requiem of Silence.mp4`,`${URL}Wishing.mp4`,`${URL}Yuki no hate ni Kimi no na wo.mp4`,`${URL}Door.mp4`,`${URL}What you don't know.mp4`,`${URL}I Trust You.mp4`,];videoUrls=t,newvideoUrls=l,window.clickCount%2==1?playVideo(newvideoUrls[0]):playVideo(videoUrls[0]),showToast(`Đ\xe3 kh\xf4i phục danh s\xe1ch ph\xe1t mặc định.`);return}let a=(JSON.parse(localStorage.getItem("playlists_"+PLAYLIST_VERSION))||{})[e];a&&a.songs&&(window.clickCount%2==1?playVideo((newvideoUrls=a.songs.map(e=>convertToUrl(e)))[0]):playVideo((videoUrls=a.songs.map(e=>convertToUrl(e)))[0]),showToast(`Đ\xe3 tải danh s\xe1ch ph\xe1t "${e}" với ${a.songs.length} b\xe0i h\xe1t.`))}function convertToUrl(e){let t=MASTER_SONGS.find(t=>t.id===e);if(t)return`${URL}${t.filename}`}function populateSidebarPlaylists(){let e=JSON.parse(localStorage.getItem("playlists_"+PLAYLIST_VERSION))||{},t=document.getElementById("dynamic-playlists");t.innerHTML="",Object.keys(e).forEach(l=>{let a=e[l];if(!a.songs||0===a.songs.length)return;let n=document.createElement("li");n.innerHTML=`
            <div onclick="loadPlaylist('${l}')" class="flex space-y-1 items-center p-2 pl-11 w-full text-base font-normal text-gray-900 rounded-lg transition duration-75 group hover:bg-blue-100">
                ...
                <div class="flex gap-1">
                    <button onclick="event.stopPropagation(); editPlaylist('${l}')" class="text-blue-500 hover:text-blue-700" title="Chỉnh sửa">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                        </svg>
                    </button>
                    <button onclick="event.stopPropagation(); deletePlaylist('${l}')" class="text-red-500 hover:text-red-700" title="X\xf3a">
                        <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
        ...
        `,t.appendChild(n)})}function createNewPlaylist(){let e=prompt("T\xean danh s\xe1ch ph\xe1t:");if(e&&e.trim()){let t=JSON.parse(localStorage.getItem("playlists_"+PLAYLIST_VERSION))||{};t[e.trim()]={songs:[],created:new Date().toISOString(),description:""},localStorage.setItem("playlists_"+PLAYLIST_VERSION,JSON.stringify(t)),populateSidebarPlaylists(),showToast(`Đ\xe3 th\xeam danh s\xe1ch ph\xe1t "${e}" th\xe0nh c\xf4ng.`),editPlaylist(e.trim())}}function deletePlaylist(e){if(confirm(`X\xf3a danh s\xe1ch "${e}"?`)){let t=JSON.parse(localStorage.getItem("playlists_"+PLAYLIST_VERSION))||{};delete t[e],showToast(`Đ\xe3 x\xf3a danh s\xe1ch "${e}" th\xe0nh c\xf4ng!`,"success"),localStorage.setItem("playlists_"+PLAYLIST_VERSION,JSON.stringify(t)),populateSidebarPlaylists()}}function editPlaylist(e){let t=JSON.parse(localStorage.getItem("playlists_"+PLAYLIST_VERSION))||{},l=t[e];if(currentEditingPlaylist=e,tempPlaylistSongs=[...l.songs],window.isEditingPlaylist=!0,!l)return;let a=createPlaylistEditorModal(e,l);document.body.appendChild(a),setTimeout(()=>a.style.opacity="1",10)}function createPlaylistEditorModal(e,t){let l=document.createElement("div");return l.className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 transition-opacity duration-300",l.style.opacity="0",l.id="playlist-editor-modal",l.innerHTML=`
        <div class="bg-white dark:bg-gray-800 rounded-lg p-6 max-w-6xl w-full mx-4 h-[90vh] overflow-hidden shadow-2xl flex flex-col">
            <div class="flex justify-between items-center mb-6 flex-shrink-0">
                <div class="flex-1 mr-4">
                    <h2 class="text-xl font-bold text-gray-900 dark:text-white mb-3">Chỉnh sửa danh s\xe1ch</h2>
                    <div class="flex items-center gap-2">
                        ...
                        <div id="playlist-name-display" class="flex items-center gap-2">
                            ...
                            <button onclick="startEditingName()" class="text-gray-500 hover:text-blue-500 transition-colors" title="Đổi t\xean">
                                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                                    <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                </svg>
                            </button>
        ...
        <div class="flex items-center justify-between p-2 rounded hover:bg-white dark:hover:bg-gray-600 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-gray-500" 
            ...
            <div class="flex items-center">
                ...
                <span class="ml-2 px-2 py-1 text-xs rounded-full ${getTypeColor(e.type)}">
                    ${getVietnameseType(e.type).toUpperCase()}
                </span>
            </div>
            <button onclick="addSongFromDataAttr(this.parentElement)" 
                    class="text-green-500 hover:text-green-700" title="Th\xeam">
                <svg class="w-4 h-4" fill="currentColor" viewBox="0 0 20 20">
                    <path fill-rule="evenodd" d="M10 3a1 1 0 011 1v5h5a1 1 0 110 2h-5v5a1 1 0 11-2 0v-5H4a1 1 0 110-2h5V4a1 1 0 011-1z" clip-rule="evenodd"></path>
                </svg>
            </button>
        </div>
    `).join("")}function addSongFromDataAttr(e){let t=e.getAttribute("data-song-id");addSongToCurrentPlaylist(t)}function removeSongFromDataAttr(e){let t=e.getAttribute("data-song-id");removeSongFromCurrentPlaylist(t)}function getVietnameseType(e){switch(e){case"opening":return"Mở đầu";case"ending":return"Kết th\xfac";case"insert song":return"Nhạc chủ đề";case"special":return"Đặc biệt";case"live concert":return"Live Concert";default:return e}}function getTypeColor(e){switch(e){case"opening":return"bg-blue-100 text-blue-800";case"ending":return"bg-green-100 text-green-800";case"insert song":return"bg-purple-100 text-purple-800";case"special":return"bg-yellow-100 text-yellow-800";case"live concert":return"bg-red-100 text-red-800";default:return"bg-gray-100 text-gray-800"}}function generateCurrentPlaylistList(e){return 0===e.length?'<div class="text-center text-gray-500 dark:text-gray-400 py-8">Danh s\xe1ch trống<br><small>Th\xeam b\xe0i h\xe1t từ cột b\xean tr\xe1i</small></div>':e.map((e,t)=>{let l=MASTER_SONGS.find(t=>t.id===e),a=l?l.title:e;return`
    ...
    `}).join("")}function getAllAvailableSongs(){return MASTER_SONGS.map(e=>e.id)}function getAllAvailableSongsLegacy(){let e=[],t=[...videoUrls,...newvideoUrls];return t.forEach(t=>{let l=cleanVideoSrcName(t);e.includes(l)||e.push(l)}),e.sort()}function addSongToCurrentPlaylist(e){tempPlaylistSongs.includes(e)||(tempPlaylistSongs.push(e),refreshModalContent())}function removeSongFromCurrentPlaylist(e){tempPlaylistSongs=tempPlaylistSongs.filter(t=>t!==e),refreshModalContent()}function refreshModalContent(){let e=document.getElementById("available-songs"),t=document.getElementById("current-playlist"),l=document.getElementById("song-search");if(e&&t){let a=l?l.value:"";e.innerHTML=generateAvailableSongsList(tempPlaylistSongs),t.innerHTML=generateCurrentPlaylistList(tempPlaylistSongs),a&&l&&(l.value=a,filterAvailableSongs());let n=document.getElementById("song-count-display");n&&(n.textContent=`${tempPlaylistSongs.length} b\xe0i`);let i=document.querySelector('button[onclick*="savePlaylist"]');i&&(0===tempPlaylistSongs.length?(i.disabled=!0,i.className=i.className.replace("bg-blue-500","bg-gray-400"),i.className=i.className.replace("hover:bg-blue-600","cursor-not-allowed")):(i.disabled=!1,i.className=i.className.replace("bg-gray-400","bg-blue-500"),i.className=i.className.replace("cursor-not-allowed","hover:bg-blue-600")))}}function closePlaylistEditor(){let e=document.getElementById("playlist-editor-modal");e&&(e.style.opacity="0",setTimeout(()=>{e.remove(),window.isEditingPlaylist=!1},300))}function savePlaylist(){let e=getCurrentPlaylistName(),t=currentEditingPlaylist;if(!e){showToast("T\xean danh s\xe1ch ph\xe1t kh\xf4ng được để trống!","error");return}if(0===tempPlaylistSongs.length){let l=JSON.parse(localStorage.getItem("playlists_"+PLAYLIST_VERSION))||{};delete l[t],localStorage.setItem("playlists_"+PLAYLIST_VERSION,JSON.stringify(l)),closePlaylistEditor(),populateSidebarPlaylists(),showToast(`Đ\xe3 x\xf3a danh s\xe1ch trống "${t}"`,"warning");return}let a=JSON.parse(localStorage.getItem("playlists_"+PLAYLIST_VERSION))||{};if(e!==t){if(a[e]){showToast(`Danh s\xe1ch "${e}" đ\xe3 tồn tại!`,"error");return}a[e]={songs:[...tempPlaylistSongs],created:a[t].created,description:a[t].description||"",modified:new Date().toISOString()},delete a[t],showToast(`Đ\xe3 đổi t\xean từ "${t}" th\xe0nh "${e}" v\xe0 lưu th\xe0nh c\xf4ng!`)}else a[e].songs=[...tempPlaylistSongs],a[e].modified=new Date().toISOString(),showToast(`Đ\xe3 lưu th\xe0nh c\xf4ng danh s\xe1ch ph\xe1t "${e}"`);localStorage.setItem("playlists_"+PLAYLIST_VERSION,JSON.stringify(a)),populateSidebarPlaylists(),closePlaylistEditor()}